(function () {
  ...
  function createCard(product) {
    ...
    card.innerHTML = `
      <div class="card-image">
        ...
        <div class="card-image-scrim" aria-hidden="true"></div>
        <span class="card-tag">${product.category}</span>
        <button type="button" class="btn-quickadd" data-id="${product.id}" aria-label="Agregar ${product.name} al carrito">
    ...
    `;
    card.querySelector(".btn-quickadd").addEventListener("click", (event) => {
    ...
    cardImage.addEventListener("keydown", (event) => {
      if (event.target.closest(".btn-quickadd")) return;
  ...
  function renderFeatured() {
    ...
    featuredProducts.forEach((product) => featuredGrid.appendChild(createCard(product)));
  }
